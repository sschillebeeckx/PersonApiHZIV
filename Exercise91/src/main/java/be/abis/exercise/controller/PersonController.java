...
public class PersonController {
   ...
   /* @PostMapping("")
    ...
    public Mono<ResponseEntity<Object>> addPerson(@RequestBody Person p) {
        ...
        // business logic
        return ps.addPerson(p)
                .thenReturn(ResponseEntity.<ProblemDetail>noContent().build())
                .onErrorResume(PersonAlreadyExistsException.class, ex -> {
                    ProblemDetail problem = ProblemDetail.forStatus(HttpStatus.CONFLICT);
                    problem.setTitle("Person already exists");
                    problem.setDetail(ex.getMessage());
    ...
    @PutMapping("{id}")
    public Mono<Void> changePasswordviaPerson(@PathVariable("id") int id, @Valid @RequestBody Person person) {
    ...
    @PatchMapping("{id}/password")
    public Mono<Void> patchPassword(@PathVariable("id") int id, @Valid @RequestBody Password password) {
