...
import be.abis.exercise.security.ApiKeyStore;
...
public class PersonController {
    ...
    @PostMapping("login")
    public Mono<ResponseEntity<PersonDTO>> findPersonByMailAndPwd(@RequestBody Login login) {
         return ps.findPerson(login.getEmail(), login.getPassword())
                    .map(person -> {
                        String apiKey = UUID.randomUUID().toString();
                        ApiKeyStore.store(person.getPersonId(), apiKey);
    ...
    public Mono<ResponseEntity<Object>> addPerson(@RequestBody PersonForm p) {
        ...
        return ps.addPerson(PersonMapper.toPerson(p))
                .map(saved ->
                        ResponseEntity.ok().body((Object) PersonMapper.toDTO(saved))
                )
    ...
    @PutMapping("{id}")
    public Mono<PersonDTO> changePasswordviaPerson(@PathVariable("id") int id, @Valid @RequestBody PersonForm person, @RequestHeader("X-API-Key") String apiKey) {
        if (!ApiKeyStore.isValid(id, apiKey)) {
            return Mono.error(new ApiKeyWrongException("Invalid API key"));
        }
    ...
    public Mono<PersonDTO> findPerson(@RequestParam("email") String email) {
        Mono<Person> person = ps.findPersonByEmail(email);
        return person.map(PersonMapper::toDTO);
    }
