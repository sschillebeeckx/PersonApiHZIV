...
public class MyExceptionHandler implements ErrorWebExceptionHandler {
	...
	@ExceptionHandler(value = ApiKeyWrongException.class)
	protected Mono<ProblemDetail> handleApiKeyWrong(ApiKeyWrongException awe, ServerWebExchange exchange) {
		ProblemDetail problem = ProblemDetail.forStatus(HttpStatus.UNAUTHORIZED);
		problem.setTitle("API key problem");
		problem.setDetail(awe.getMessage());
		return Mono.just(problem);
	}
